...
/*     Get.find<controllertext>(tag: 'first').notes.clear();
  ...
  Future<dynamic> alterTable(String ColumneName) async {
    var dbClient = await instance.database;
    var count = await dbClient.execute("ALTER TABLE PhonesDatabase8 ADD "
        "COLUMN $ColumneName TEXT;");
    print('$ColumneName');
    //  print(await dbClient.query('EhsanDatabase'));
    return count;
  }
  ...
  Future<List<phones>> phoness() async {
    ...
    return List.generate(maps.length, (i) {
      return phones(
          ...
          HalfOnRemote: maps[i]['HalfOnRemote'],
          TypeMicro: maps[i]['TypeMicro']);
    });
  }

  Future<void> deletephone(phones id) async {
